...
// ensure storage methods are stubbed out during unit tests
jest.mock('../src/app/services/storage.service', () => ({
  default: {
...
describe('ProductService (mocked)', () => {
  ...
  test('createProductToDB should create and return product', async () => {
    ...
    const res = await ProductService.createProductToDB({
      title: 'Brake Pad',
    } as any);
  ...
  test('updateProductToDB should update and return product', async () => {
    // mock existing product so deletion logic doesn't throw
    ...
    const res = await ProductService.updateProductToDB('prod1', {
      price: 12,
    } as any);
  ...
  test('deleteProductFromDB should delete product or throw if not found', async () => {
    ...
    await expect(
      ProductService.deleteProductFromDB('prod1'),
